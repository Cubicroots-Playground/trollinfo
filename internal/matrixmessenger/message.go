...
func (messenger *service) SendMessageAsync(ctx context.Context, message *Message) error {
	go func() {
		_, _ = messenger.sendMessage(ctx, message.toEvent(), message.ChannelExternalIdentifier, 10, time.Second*10)
	}()
...
func (messenger *service) SendMessage(ctx context.Context, message *Message) (*MessageResponse, error) {
	return messenger.sendMessage(ctx, message.toEvent(), message.ChannelExternalIdentifier, 3, time.Second*5)
}
...
func (messenger *service) sendMessage(ctx context.Context, messageEvent *messageEvent, channel string, retries uint, retryTime time.Duration) (*MessageResponse, error) {
	var err error
	maxRetries := retries
	...
	for retries > 0 {
		...
		response, err := messenger.sendMessageEvent(ctx, messageEvent, channel, messageEvent.getEventType())
		if err == nil {
			...
			messenger.logger.Infof("Sending message is stopped since we ran in a rate limit")
			continue
	...
	messenger.logger.Infof("Sending message failed and retries are exceeded. Error is: " + err.Error())
	return nil, err
}
